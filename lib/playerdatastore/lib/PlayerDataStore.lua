...
-- player data.
local DATASTORE_NAME = 'PlayerDataStore_PlayerData_RELEASE00'
...
function SaveData.new(playerDataStore, userId)
	...
	function this:Set(key, value, allowErase)
		me.debugPrint("(".. this.userId .. ") SETTING " .. tostring(key) .. " TO " .. tostring(value));
		if type(key) ~= 'string' then
...
function PlayerDataStore.new()
	...
	local function doLoad(userId)
		...
		if mOnRequestUserIdSet[userId] then
			...
			-- load the data
			local data = nil;
			local dataRequest = pcall(function()
				data = mDataStore:GetAsync(userIdToKey(userId)) or {}
			end);
			if not dataRequest then
				-- we need to constantly wait for roblox servers to respond.
				local function retry()
					pcall(function()
						data = mDataStore:GetAsync(userIdToKey(userId)) or {}
					end);
				end
				repeat print("[DATA] !!! ROBLOX SERVERS FAILED FOR ".. userId ..". TRYING AGAIN IN 5 SECONDS !!"); wait(5) retry() until data ~= nil;
			end
